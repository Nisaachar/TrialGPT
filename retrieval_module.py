...
def get_keyword_generation_messages(note, max_keywords):
    ...
    system = (
        ...
        f'Please generate up to {max_keywords} key conditions or keywords for searching relevant clinical trials for this patient.'
        ...
        'The key condition list should be ranked by priority. Output only a JSON dict formatted as Dict{"conditions": List[Str(condition)]}. Don\'t output anything else.'
    )
...
def hybridRetriever():
    ...
    print(f"Elapsed time for retrieval: {elpased_time:.2f} seconds")
